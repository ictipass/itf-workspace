...
export const ADMIN_HELP_TOPICS: readonly AdminHelpTopic[] = [
  {
    ...
    entries: [
      ...
      },
      {
        ...
        summary: "Use a saved one-time recovery code or the governed in-person assisted-recovery workflow; never bypass MFA.",
        steps: [
          "Report loss/theft immediately through the approved ICT/security channel; do not send a code, QR image or setup key.",
          "If the user saved a recovery code, have them sign in with the password, choose Use a saved recovery code, then enroll a new authenticator after the forced sign-in.",
          "Without a valid code, an appointed HR Identity Verifier checks the person in person and creates the request in MFA Recovery.",
          "For STAFF, a different SYSTEM_ADMIN executes it. For APP_ADMIN/SYSTEM_ADMIN, a distinct ICT Security Approver must approve before a different SYSTEM_ADMIN executes.",
          "For the only locked-out SYSTEM_ADMIN, the appointed ICT Recovery Operator executes after HR and ICT Security approval.",
          "Confirm all Workspace/connected-app sessions ended, the new authenticator was enrolled and the security notification was delivered or separately escalated.",
        ],
        href: "/dashboard/admin/mfa-recovery",
        linkLabel: "Open MFA Recovery",
        escalation: "Do not continue if the required independent authority is unavailable, the request expired, notification failed or child-session revocation cannot be verified.",
      },
      {
        title: "Move to another phone or authenticator app while the old one still works",
        summary: "Workspace replaces the server-side TOTP secret only after the password, old code and new code are verified.",
        steps: [
          "Keep the old authenticator until replacement finishes, then open Authenticator Security.",
          "Enter the current Workspace password and a fresh code from the old authenticator.",
          "Scan the new QR/setup key privately and enter a code from the new authenticator.",
          "Store the newly displayed recovery codes; the old factor/codes and every current session are invalidated.",
          "Sign in again and confirm the official-email security notice.",
        ],
        href: "/dashboard/security/authenticator",
        linkLabel: "Open Authenticator Security",
        escalation: "Cloud backup is allowed only under an ICT-approved provider/account/device profile. Restoring a copied secret does not revoke a lost copy.",
      },
    ],
  },
  {
