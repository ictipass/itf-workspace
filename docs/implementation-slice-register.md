...
|---|---|---|---|
| A01 | ITF Flow launch v2, provisioning and revocation integration | In progress | Code complete: Workspace `1a08a5b`, Flow `02b433d`; Flow staging configuration `f48b702` and pooled/direct database hardening `d0199ce`. Database credential rotation and the dedicated staging deployment-protection correction are confirmed. Independent probes confirm public Flow readiness/launch routing and acceptance of the installed Flow-side directory/interoperability credentials without mutation. Workspace staging sender/worker configuration, registry/access setup, joint lifecycle acceptance and a continuous retry scheduler remain. See [`slices/A01-itf-flow-integration-reassessment.md`](slices/A01-itf-flow-integration-reassessment.md) |
...
**Next best implementable slice:** continue A01 by installing Flow endpoint values and the matching directory and
interoperability credentials in Workspace staging, plus its independent outbox-worker credential. Redeploy Workspace,
register/grant Flow, synchronize the directory, and execute joint acceptance. The continuous retry scheduler remains a
...
boundary and receiver credentials are confirmed. Workspace's staging sender/worker configuration, registry/access
records and full joint lifecycle evidence remain.
