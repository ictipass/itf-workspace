...
| D05 | Required MFA methods and roles/actions that require step-up authentication | Approved (interim) | Super administrator | Password-only for explicitly standard apps/roles; TOTP step-up when an app or assigned role is sensitive; `SYSTEM_ADMIN` and `APP_ADMIN` are always sensitive; ten-minute freshness; email is not MFA. `2026-08-23`, project architecture directive follow-up |
...
| D07 | Production signing model, key custody, rotation interval and emergency rotation process | Approved (interim) | Super administrator; ICT Security policy owner; Infrastructure Operations custodian | RS256/3072-bit; vendor-neutral signer; non-exportable production KMS/HSM; 90-day rotation; 24-hour normal public-key overlap; emergency no-overlap rotation; versioned JWKS and notifications. `2026-08-23`, [`launch assertion signing-key policy`](policies/2026-08-23-launch-assertion-key-management-policy.md) |
...
| D13 | Production hosting, reverse proxy/CDN, TLS termination and trusted forwarded-header topology | Open (partial topology recorded) | ITF infrastructure | Vercel is the selected host; Workspace staging is a Preview deployment tied to the `staging` branch and `itf-workspace-staging.vercel.app`. ITF Flow staging uses a dedicated `itf-flow-staging` project with Vercel Authentication disabled so application-level browser launch and service authentication can operate; its intended production deployment is separately identified as `itf-flow.vercel.app`. This staging choice does not set production policy. Production proxy/TLS ownership, deployment protection, regions, operational access and Vercel OIDC/external-KMS topology remain open. `2026-09-05`, [`Vercel deployment environments`](vercel-deployment-environments.md) |
...
| D38 | Difference between child-app logout and global Workspace logout | Approved (interim) | Super administrator | A child app's main logout terminates only that child session and returns to the Workspace catalogue. An adjacent menu offers global logout; Workspace confirms the action, terminates the current Workspace session and propagates logout to all connected child apps bound to that session. Other Workspace sessions/devices remain active. `2026-09-08`, [`session exit-scope directive`](policies/2026-09-08-session-exit-scope-directive.md) |
| D39 | Staff entry from a child application's public or login page | Approved (interim) | Super administrator | Deployed child apps that do not authenticate staff locally provide a clear link to Workspace login from their public landing and staff-access pages. Development/demo local login remains controlled by its existing environment policy; external stakeholder identity routes remain separate. `2026-09-08`, [`child-app staff-entry directive`](policies/2026-09-08-child-app-staff-entry-directive.md) |
| D40 | Permitted organization reference-data bulk-import behavior and controls | Approved (interim) | Super administrator | One exact five-sheet XLSX or five named CSVs; create with blank `recordId`, update by exported immutable ID; explicit active-state changes permitted; no delete, omission effect or hierarchy move; mandatory dry run, digest-bound receipt, fresh TOTP, atomic apply and audit. `2026-09-09`, [`organization reference-data bulk-import directive`](policies/2026-09-09-organization-reference-data-import-directive.md) |
